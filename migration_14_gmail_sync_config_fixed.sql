-- Migration 14: Configuración específica de sincronización de Gmail (FIXED)
...
DROP TABLE IF EXISTS gmail_sync_config CASCADE;

-- PASO 2: Eliminar función helper si existe
DROP FUNCTION IF EXISTS initialize_gmail_sync_config(UUID) CASCADE;

-- PASO 3: Crear tabla de configuración de Gmail
CREATE TABLE gmail_sync_config (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,

    -- Configuración de sincronización
    max_emails_per_sync INTEGER DEFAULT 200,
    initial_sync_days INTEGER DEFAULT 15,
    sync_frequency_minutes INTEGER DEFAULT 60,

    -- Filtros de sincronización
    excluded_labels TEXT[] DEFAULT ARRAY[]::TEXT[],
    included_labels TEXT[] DEFAULT NULL,
    ...
    max_email_content_length INTEGER DEFAULT 50000,
...
CREATE INDEX idx_gmail_sync_config_user_id ON gmail_sync_config(user_id);
CREATE INDEX idx_gmail_sync_config_watch_enabled ON gmail_sync_config(user_id) WHERE watch_enabled = true;
...
ON gmail_sync_config FOR SELECT
USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own gmail config"
ON gmail_sync_config FOR INSERT
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own gmail config"
ON gmail_sync_config FOR UPDATE
USING (auth.uid() = user_id);

CREATE POLICY "Users can delete own gmail config"
ON gmail_sync_config FOR DELETE
USING (auth.uid() = user_id);

CREATE POLICY "Service role has full access to gmail config"
ON gmail_sync_config FOR ALL
USING (auth.role() = 'service_role');

-- PASO 7: Crear trigger para updated_at (reutiliza función de migration_13)
...
-- PASO 8: Crear función helper para inicializar config
